...
if (strpos($url, "$adminZone") === 0) {
    ...
    if (substr($url, -4) === '.css')  header('content-type: text/css');
    if (substr($url, -3) === '.js')  header('content-type: text/js');
    echo $content;
    exit();
}
...
$n = new Namaskar(MEMPAD_FILE);

if (defined("MEMPAD_CONF_FILE"))
    $n->configFile(constant("MEMPAD_CONF_FILE"));
